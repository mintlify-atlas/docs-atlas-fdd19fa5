---
title: "Get started with maxmind-geolite-mirror"
...
<Steps>
  ...
  <Step title="Run the mirror">
    ...
    The tool processes each database in series. For each file it first sends a `HEAD` request with an `If-Modified-Since` header. If the remote file is newer (or the local file does not yet exist), it downloads and extracts the tarball. You will see output similar to:
